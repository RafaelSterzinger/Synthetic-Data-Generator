...
def plot_validation(history: History):
    plt.figure(figsize=(8, 5))
    plt.plot([i + 1 for i in range(len(history.epoch))], history.history['val_acc'], '-', c='orange', lw=2, markersize=9)
    plt.grid(True)
...
def plot_validation_loss(history: History):
    plt.figure(figsize=(8, 5))
    plt.plot([i + 1 for i in range(len(history.epoch))], history.history['val_loss'], '-', c='orange', lw=2, markersize=9)
    plt.grid(True)
